...
use mcp_dap::tools::execution::{ContinueParams, StepGranularity, StepParams};
...
async fn test_mutex_step_timeout() {
    ...
    let step_over_println = server
        .handle_step(StepParams {
        ...
        .await
        .expect("step over println should succeed");
    ...
    assert!(
        step_over_mutex.is_err() || step_over_mutex.unwrap().is_error == Some(true),
