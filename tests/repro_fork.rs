...
use mcp_dap_rs::tools::execution::ContinueParams;
...
async fn test_fork_breakpoint() {
    ...
    let result = server.handle_evaluate(mcp_dap_rs::tools::inspect::EvaluateParams {
        expression: "is_static".into(),
        frame_id: None,
        context: None,
    }).await.expect("evaluate is_static should succeed");
    ...
    let result = server.handle_evaluate(mcp_dap_rs::tools::inspect::EvaluateParams {
        ...
        frame_id: None,
        context: None,
    }).await.expect("evaluate notif.data.nr should succeed");
    ...
    }).await.expect("repl evaluate should succeed");
